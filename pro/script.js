...
signUpBtnEle.addEventListener("click", function (e) {
  ...
  if (!password) {
    signUpPasswordEle.setAttribute(
      "placeholder",
  ...
  blogModelBtnEle.classList.remove("active_ele");
  navCurrentUser.classList.remove("active_ele");
  loginUserImg.classList.remove("active_ele");
...
signInBtnEle.addEventListener("click", function (e) {
  ...
  if (!password) {
    signInUserPasswordEle.setAttribute(
      "placeholder",
      "Please Enter Your Password!"
    );
  ...
  const user = signUpUsers.find((u) => u.email === email);
  if (!user) {
...
logoutBtnEle.addEventListener("click", function (e) {
  e.preventDefault();
  const modalEl = document.getElementById("exampleModalLogout");
  const modal =
    bootstrap.Modal.getInstance(modalEl) || new bootstrap.Modal(modalEl);
  deleteCookie("loginUser");
  let currentLoginUser = JSON.parse(getCookie("loginUser") || "{}");
  if (!currentLoginUser.userName) {
    ...
    loginUserImg.classList.add("active_ele");
